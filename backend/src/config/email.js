...
const enviarCodigoVerificacion = async (destinatario, codigo, nombre) => {
  const mailOptions = {
    from: `"Burroomies" <${process.env.BREVO_EMAIL_SENDER}>`,
    to: destinatario,
    subject: 'Verifica tu cuenta - Burroomies',
    html: `
      <!DOCTYPE html>
      <html>
      <head>
...
const enviarCorreoContacto = async (nombre, email, mensaje) => {
  const mailOptions = {
    from: `"Burroomies Contacto" <${process.env.BREVO_EMAIL_SENDER}>`,
    ...
    html: `
      ...
      <body style="font-family: Arial, sans-serif; background-color: #f4f4f4; margin: 0; padding: 20px;">
        <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 10px; padding: 20px; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
          ...
          <p style="font-size: 12px; color: #999; text-align: center;">
            Este mensaje fue enviado desde el formulario de contacto de Burroomies
          </p>
        </div>
      </body>
      </html>
